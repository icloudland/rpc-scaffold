...
var (
	...
	ErrClientResponseTimeOut = errors.New("websocket client wait response time out")
)
...
type (
	...
	rawResponse struct {
		...
		Error  *rpcjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled rpcjson.RPCError (or an unmarshaling error) if so.
...
func receiveFutureTimeOut(f chan *response, seconds uint) ([]byte, error) {
	if seconds == 0 {
		return receiveFuture(f)
	}

	timeOuter := time.NewTimer(time.Second * time.Duration(seconds))
	select {
	case <-timeOuter.C:
		return nil, ErrClientResponseTimeOut
	case r := <-f:
		return r.result, r.err
	}
}
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := rpcjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := rpcjson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
...
func newHTTPClient(config *ConnConfig) (*http.Client, error) {
	...
	if !config.DisableTLS {
		...
		cert, _ := tls.LoadX509KeyPair(config.RpcCert, config.RpcKey)
